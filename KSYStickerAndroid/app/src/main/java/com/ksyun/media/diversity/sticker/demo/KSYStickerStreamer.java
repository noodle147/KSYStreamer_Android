...
public class KSYStickerStreamer {
    ...
    private KSYStickerStreamer.OnInfoListener mOnInfoListener;
    private KSYStickerStreamer.OnErrorListener mOnErrorListener;
    ...
    private ImgStickerProcessor mImgStickerProcessor;
    private ImgBufScaleFilter mImgBufScaleAndConvertFilter;
    ...
    private STMobileStickerNative mSTMobileStickerNative;

    public KSYStickerStreamer(Context context) {
        if (context == null) {
    ...
    private void initModules() {
        ...
        // create objects
        mImgBufScaleAndConvertFilter = new ImgBufScaleFilter();
        mImgBufScaleAndConvertFilter.setMirror(true);
        mImgBufScaleAndConvertFilter.setOutputFormat(ImgBufFormat.FMT_RGBA);

        mImgYFlipFilter = new ImgYFlipFilter(mGLRender);

        mImgStickerProcessor = new ImgStickerProcessor(mGLRender);
        mImgTexFilterMgt.getSrcPin().connect(mImgYFlipFilter.getSinkPin());
        mImgYFlipFilter.getSrcPin().connect(mImgStickerProcessor.getTexSinkPin());
        mImgStickerProcessor.getSrcPin().connect(mImgTexMixer.getSinkPin(0));
        ...
        //scale and convert from nv21 to rgba
        mCameraCapture.mImgBufSrcPin.connect(mImgBufScaleAndConvertFilter.getSinkPin());
        mImgBufScaleAndConvertFilter.getSrcPin().connect(mImgStickerProcessor.getBufSinkPin());
    ...
    private void updateFrontMirror() {
        if (mCameraFacing == CameraCapture.FACING_FRONT) {
            ...
            mImgBufScaleAndConvertFilter.setMirror(false);
        }
    }
    ...
    private void setPreviewParams() {
        ...
        mImgBufScaleAndConvertFilter.setTargetSize(mTargetWidth, mTargetHeight);
    ...
    public void setStickerAndModulePath(String stickerPath, String modulePath) {
        mImgStickerProcessor.setStickerPathAndModulePath(stickerPath, modulePath);
    }
     ...
     */
    public void setOnInfoListener(KSYStickerStreamer.OnInfoListener listener) {
     ...
     */
    public void setOnErrorListener(KSYStickerStreamer.OnErrorListener listener) {
    ...
    public void hideSticker() {
        if (mImgStickerProcessor != null) {
            mImgStickerProcessor.releaseSticker();
        }
    }

    public void showSticker() {
        if (mImgStickerProcessor != null) {
            mImgStickerProcessor.showSticker();
        }
    }

    public int changeSticker(String path) {
        if (mImgStickerProcessor != null) {
            return mImgStickerProcessor.changeSticker(path);
        } else {
            return 0;
        }
    }
